...
Three under-served gaps motivate the contribution. First, there is no head-to-head sub-4B safety comparison under a common protocol; published comparisons hold the attack distribution near-fixed and vary the defense [@xie2023selfreminder], hold the defense near-fixed and vary the attack [@mazeika2024harmbench], or hold the threat model near-fixed and vary the over-refusal category [@cui2024orbench]. We hold all three near-fixed and vary the model. Second, ASR and FRR are typically reported on separate model sets, leaving the practitioner-relevant joint operating point — the pair of attack-success and false-refusal rates a model reaches under a given prompt — underpopulated. Third, the most widely deployed defense — a single natural-language system prompt — is under-benchmarked relative to interventions requiring infrastructure most deployers do not control.
