...
Every model and judge is pulled at a pinned Hugging Face revision recorded in `run_manifest.json`. Every dataset loader threads an explicit `revision` argument, to be pinned to commit SHAs at camera-ready (the shipped specs currently default to upstream `main`; see Appendix E.1). The verbatim defensive prompts and their SHA-256 hashes are publicly pre-registered (committed and tagged in the public repository before any run). The harness is released as a single `uv`-installable Python package; one shell script reproduces every table and figure in the paper. Package versions are stated in `requirements.txt` (with the packages lacking Python 3.13 wheels floored to their first cp313-compatible release) and looser bounds in `requirements-loose.txt`; these are the versions the camera-ready run is executed against. Gated model access (Llama-3.2-3B-Instruct, Gemma-2-2B-it, Llama-Guard-3-1B) is detected and reported with an explicit message instructing the user to run `huggingface-cli login` and accept the licenses.
...
The verbatim primary prompt and two pre-registered variants are published in Appendix B and committed and tagged in the public repository before the main experiments are run. Each prompt is SHA-256-hashed at load and the hash is recorded in `run_manifest.json`.
...
Figure 1 plots all four models in the (ASR, FRR) plane under the empty system prompt and under the primary defensive system prompt, aggregated across the three judges with a shaded judge-variance band. Within each model, an arrow connects the empty-prompt operating point to the defensive-prompt operating point; the direction and length of the arrow visualise the per-model defense effect.

**Table 1.** Joint (ASR, FRR) operating points per (model, system prompt). ASR is the mean across all three judges; FRR is the mean across the two FRR judges (keyword and Llama-Guard-3-1B), per Section 4.6.
...
Three forward extensions follow. Multi-turn scope; multilingual scope; and the 1B class. We commit a refresh appendix for the camera-ready that adds at least one post-2024 sub-4B reference model (Phi-4-mini, SmolLM2, or OLMo-2-1B) so that the protocol does not date as the on-device class evolves. The harness, the signed pre-registration tag, the pre-registered defensive prompts, and the one-page practitioner checklist (Appendix D) are released alongside the paper.
...
- **Pre-registration.** The pre-registered protocol, harness commit hash at pre-registration time, model and judge revision pins, defensive prompt SHA-256 hashes, and analysis script's expected output schema are frozen in `08_preregistration.md`, committed under a signed Git tag and posted as an arXiv v1 of the protocol before any main experimental run, so the registration carries an independent public timestamp. The tag name and arXiv identifier are populated in Section 5.7 and Appendix I before submission.
...
**Distribution.**
- The default-released files (`results.csv`, `aggregate.csv`, `run_manifest.json`, `asr_frr.png`) are distributed alongside the paper and the pre-registration (the signed protocol tag and arXiv v1). They contain labels and statistics only.
...
**Pre-registration.** The frozen protocol, revision pins, and defensive-prompt SHA-256 hashes in `08_preregistration.md` are committed under a signed Git tag and posted as an arXiv v1 of the protocol (arXiv ID: `TBD-on-posting`) for an independent public timestamp. The registration timestamp must precede the first `run_manifest.json` timestamp.
