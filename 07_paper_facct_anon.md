...
*Affiliation note.* This paper reports an independent comparative audit by Responsible AI practitioners. No internal or proprietary models, datasets, evaluation tooling, or deployment systems were used; all target models, judges, and benchmark prompts are drawn from publicly released artifacts at pinned revisions.
...
The contribution is deliberately framed as *audit not capability*: no new attack, no new defense, no new benchmark — only measurement on previously published benchmarks at pinned revisions. We develop this framing and its alignment with ACM FAccT and an established Responsible AI standard [reference withheld for review] in Section 7.6. The deliverable is operational: a practitioner picks a model, paste the defensive system prompt, runs one command, reads the joint frontier, and verifies the ranking under at least two judges before shipping. The harness, the pinned revisions, the three pre-registered defensive prompt variants, and the analysis script are released alongside the paper.
...
By default, the harness persists per-prompt per-judge verdicts (one row per (model, condition, benchmark, prompt_id, judge) tuple) and aggregate statistics. Raw model completions are not written to disk. An opt-in encrypted-completions sink (`--store-completions`) is available for reviewer-audit purposes; completions are written under Fernet symmetric encryption to a path inside an access-controlled, author-managed environment, with bounded retention and a documented reviewer-access protocol. The opt-in is OFF by default; the prose, code, and pre-registration all match this behaviour. The opt-in path refuses to write completions in plaintext.
...
The floor-vs-ceiling argument and its three-part justification are stated in the threat model (Section 3.7); we do not restate them here. One discussion-level addition: defensive system prompts are necessary but never sufficient. We recommend layered defenses (Llama-Guard-3 on input and output where infrastructure permits, classifier auxiliaries where the inference stack allows, fine-tuning interventions where weights access exists) and frame our work as the audit of the lowest layer of that stack.
...
### 8.5 Methodological safeguards against author bias

One of the four target models shares a vendor with the authors; we disclose this potential conflict here, and the full competing-interests statement is provided in the camera-ready endmatter (withheld for double-blind review). Mitigations are five-fold. (a) The public pre-registration of the protocol is timestamped before any data collection. (b) The defensive prompt content is identical across all four models; the placement asymmetry for Gemma-2 (whose chat template does not accept a system role) is disclosed in Section 4.4 and ablated in Section 6.5. (c) The three judges are triangulated and the ranking-flip rule is pre-registered. (d) An established Responsible AI standard's requirement of reproducible measurement protocols for comparative claims is followed [reference withheld for review]. (e) **Pre-commitment.** If the vendor-shared model is the safety-frontier model under any judge, we (i) report the result with the same emphasis and confidence treatment as for any other model, (ii) explicitly attempt to find a judge or condition under which it is not, and (iii) include the negative-result attempt in the paper. The reverse pre-commitment also holds: if the vendor-shared model is the worst-performing model under any judge, we report that with the same emphasis. The pre-commitment is logged in the public pre-registration.
...
- *Who created it and who funded it?* The authors; see the conflict-of-interest disclosure in Section 8.5.
...
- The encrypted-completions sink is **not** distributed publicly; per Sections 5.6 and 8.2 the documented path lives inside an access-controlled, author-managed environment under a documented key-management and reviewer-access protocol, with bounded retention, and reviewers may request access via the program chairs.

**Maintenance.**
- *Maintainer:* the corresponding author [anonymized for review].
...
- *Conflict of interest:* one of the four target models shares a vendor with the authors; mitigations (pre-registration before data collection, identical prompt content across models, judge triangulation, a pre-registered ranking-flip rule, and a symmetric pre-commitment) are in Section 8.5.
